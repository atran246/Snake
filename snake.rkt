...
(define INITIAL-FOOD (make-food (* SCALE (+ (random MAX) 1)) 
                                (* HEIGHT (+ (random MAX) 1))))
...
; move the worm based on its direction
(define (move-worm* gs)
  (make-game (move (game-worm gs)) 
             (if (empty? (game-low gs)) empty
                 (cons (game-worm gs) (move-worm (game-low gs))))
             (game-food gs)
             (game-score gs)))
...
(define (change-dir* gs cmd)
  (make-game (change-dir (game-worm gs) cmd) 
             (game-low gs) 
             (game-food gs)  
             (game-score gs))) 
...
(define (change-worm-dir low)
  (cond 
    [(empty? (rest low)) empty]
    [(cons? (rest low)) (cons (first (rest low)) 
                              (change-worm-dir (rest low)))]))
...
; worm -> worm
; determine if the worm has hit the top of the background
(define (hit-top s)
  (<= (posn-y (worm-pos s)) BODY-RADIUS))

; worm -> worm
; determine if the worm has hit the bottom of the background
(define (hit-bottom s)
  (>= (posn-y (worm-pos s)) BACKGROUND-HEIGHT)) 

; worm -> worm
; determine if the worm has hit the left of the background
(define (hit-left s)
  (<= (posn-x (worm-pos s)) BODY-RADIUS)) 

; worm -> worm
; determine if the worm has hit the right of the background
(define (hit-right s)
  (>= (posn-x (worm-pos s)) BACKGROUND-WIDTH)) 

; worm -> worm
; determine if the worm has collided with a wall
...
(define (collided? g)
   (cond 
       [(detect-collision g) true]
       [(empty? (game-low g)) false]
       [(= (+ (worm-dir (game-worm g)) (worm-dir (first (game-low g)))) 3) true]
       [(hit-self? (game-worm g) (game-low g)) true]
       [else false]
       ))
...
(define (update-game gs)
  (if (eating-food? gs) (add-segment gs)
...
(define (render-worm-death s )
  (place-image FOOD 
               (food-x (game-food s)) (food-y (game-food s))
         (place-image HEAD
               (posn-x (worm-pos (game-worm s))) (posn-y (worm-pos (game-worm s)))
         (overlay/align "middle" "middle" (text  "You Lose :( Score: 0" 36 "White") 
          BACKGROUND))))
...
(define (render-game s)
  (if (empty? (game-low s)) (render-worm s)
      (let* ([gws (game-low s)]
        ...
        (place-image FOOD fx fy 
              (place-image HEAD wx wy
                     (cond 
                       [(empty? (rest gws)) (place-image (circle BODY-RADIUS "outline" 
                                                                 (list-ref (list "turquoise" "magenta" "gold" 
                                                                                 "red" "blue" "green" "pink" "purple" "orange") (random 9)))
                                                         (posn-x (worm-pos (first (game-low s)))) (posn-y (worm-pos (first (game-low s))))
                                                         BACKGROUND)]
                       [else (place-image (circle BODY-RADIUS "outline" (list-ref (list "turquoise" "magenta" "gold" "red" "blue" "green" "pink" "purple" "orange"
                                                                                      ) (random 9)))  
                                          (posn-x (worm-pos (first (game-low s)))) (posn-y (worm-pos (first (game-low s)))) 
